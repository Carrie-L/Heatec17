...
public class SystemMethod {
	...
	public static void addToCalendar(Context mContext) {
		try {
			...
			DialogUtils
					.showAlertDialog(mContext, R.string.addToCalendar_success,
							R.string.ok, -1, null, null);

		} catch (Exception e) {
			e.printStackTrace();
			DialogUtils.showAlertDialog(mContext, R.string.addToCalendar_fail,
					R.string.ok, -1, null, null);
		}
	}
	 ...
	 *            下载地址
	 * @param targetPath
	 *            文件存放的目标路径 /mnt/sdcard/com.adsale.ChinaPlas/Seminar/
	 * @param zipPath
	 *            zip包的路径 , e.g.:
